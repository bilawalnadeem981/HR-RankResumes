...
import { Search, Bell, Settings, ChevronDown, HelpCircle, Shield } from "lucide-react";
    ...
    <header className="bg-white/80 backdrop-blur-md border-b border-gray-100 sticky top-0 z-40 px-8 py-3 flex items-center justify-between">
      ...
      <div className="flex-1 max-w-xl group">
        <div className="relative">
          ...
          />
          <div className="absolute right-3 top-1/2 -translate-y-1/2 px-1.5 py-0.5 rounded border border-gray-200 bg-white text-[10px] font-black text-gray-400 uppercase tracking-tighter hidden sm:block">
            ⌘ K
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="flex items-center gap-3 pr-6 border-r border-gray-100">
          <button 
            ...
            className="flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-black uppercase tracking-wider bg-gray-50 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 transition-all border border-gray-100"
          >
            <Shield className={`w-4 h-4 ${role === 'admin' ? 'text-indigo-600' : 'text-gray-400'}`} />
